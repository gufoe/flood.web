...
Net.mutations = {
    ...
    removeNode(_) {
        ...
        _.active.remove(n.id)
        _.outputs.remove(n.id)
        return true
    },
}
...
Net.funs = {
    sum: (_, n) => {
        var tot = 0
        for (var i in n.inputs) tot+= _.nodes[i].val
        return tot
    },
    id: (_, n) => Net.funs.sum(_, n),
    avg: (_, n) => Net.funs.sum(_, n) / size(n.inputs),
    sig: (_, n) => 1 - 2 / (1 + Math.exp(-Net.funs.sum(_, n)*4)),
    // bool: x => x > 0 ? 1 : 0,
    // sign: x => x > 0 ? 1 : -1,
    // neg: x => -x,
